...
public class TableModelPacijenti extends AbstractTableModel implements Runnable {
    ...
    public Object getValueAt(int row, int column) {
        ...
        switch (column) {
            ...
            case 1:
                return p.getImePacijenta();
            case 2:
                return p.getPrezimePacijenta();
            case 3:
                return p.getEmail();
            case 4:
                return p.getTelefon();
        ...
        try {
            ...
            if (!parametar.equals("")) {
                ...
                for (Pacijent p : lista) {
                    if (p.getImePacijenta().toLowerCase().contains(parametar.toLowerCase())
                            || p.getPrezimePacijenta().toLowerCase().contains(parametar.toLowerCase())) {
                        novaLista.add(p);
                    }
                }
                lista = novaLista;
